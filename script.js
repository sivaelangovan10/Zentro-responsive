...
addbtns.forEach(btn => {
    btn.addEventListener("click" , () => {
        ...
        const price = Number(card.querySelector(".new")).textContent.replace("$","");
...
addCartButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        const price = parseFloat(product.querySelector('.new').textContent.replace('$', ''));
...
// Add checkout button to cart modal
setTimeout(() => {
    const cartModal = document.getElementById('cart-modal');
    if (cartModal) {
        ...
        const closeBtn = cartModal.querySelector('button onclick="closecart()"');
        if (closeBtn) {
            cartModal.insertBefore(checkoutBtn, closeBtn);
        }
    }
}, 100);
...
function scrolltocart() {
    document.getElementById('cart-modal').scrollIntoView({
        behavior: 'smooth'
    })
}
...
const divs = document.getElementById("scrolldiv")
...
function autoscroll() {
    ...
    if(pos >= divs.scrollHeight - divs.clientHeight) {
        pos = 0
